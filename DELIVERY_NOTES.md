...
<!-- figures:time -->
**About 4 hours 31 minutes of work**, against an eight-hour ceiling.

That is 2 sittings, not a single stretch: 2026-09-14 19:08–23:09, 2026-09-15 14:58–15:47. Counting straight through from the first mark to the last would give 20 hours 40 minutes, but that includes the night in between, and sleep is not work. 20 minutes of an enforced pause on my
AI tooling are subtracted on top of that.

Derived, not remembered: this paragraph is regenerated by `npm run report` from
`git log`, and reads as of that run — 37 commits. A gap longer than
60 minutes between commits starts a new sitting. The first stretch is
the exception: it holds no commits, because it is the reading and the
architecture done before the repository existed. It starts at the creation time
of the first working file — a file timestamp, not a commit, and the one figure
here that git cannot corroborate.
<!-- /figures:time -->

The first sitting, phase by phase. The second — the day after, spent on an
audit of the finished submission and on what it found — is in the commit log
rather than here.
...
*would* take a person and wrote the estimate down as fact. The repository's own
timestamps contradicted it immediately, and the person I was reporting to
caught it before I did. It is noted here because a delivery note that gets its
own headline number from intuition has no standing to report latency and cost.

The figure has since been wrong a second way, which is why it is now computed
rather than written: counting straight from the first file to the last commit
made a night's sleep look like nineteen hours of work. The generator now counts
sittings and says where the boundary falls.
...
Plus the unit and integration suite via `npm test`:
...
median on turns that did not follow an interruption (1100 ms) is close to the
overall one (1159 ms). 1 audible reading was dropped as impossible — claiming a
...
improved since. A turn needing no lookup sits below that at 1111 ms; a turn
needing one sits above. On 78 turns, one machine, one network.
<!-- /figures:observations -->
...
$0.0356 lands in the same place, which is the check that the arithmetic is
not out by an order of magnitude.

**Where the money actually goes.** Speech generation is 60% of the bill, and
audio in and out together are 74%. Text — the instructions, the conversation
...
**Caching does a lot of quiet work.** 207,616 of 252,096 text input tokens —
82% — were served from cache. At the uncached rate those would have cost
$0.1246 instead of $0.0125. The instructions and the tool schemas are
...
<!-- figures:tier -->
The same 15.5 recorded minutes, priced at the full
`gpt-realtime-2.1` tier instead, would have cost **$1.96 rather than
$0.55** — $0.1263 per minute against $0.0356,
or **3.5× the bill**. For scale, that is 2.5× the $0.05/min flat
