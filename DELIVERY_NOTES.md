...
<!-- figures:time -->
**3 hours 24 minutes of elapsed wall-clock time**, of which 20 minutes
was an enforced pause on my AI tooling. So about **3 hours 4 minutes of actual
work**, against an eight-hour ceiling.

Derived, not remembered: this paragraph is regenerated by `npm run report` from
`git log`. The end is the last commit, 22:32 on 2026-09-14, across 23 commits
...
overall one (1170 ms). 1 audible reading(s) were dropped as impossible — they claimed a
...
improved since. A turn needing no lookup sits below that at 1102 ms; a turn
needing one sits above. On 69 turns, one machine, one network.
<!-- /figures:observations -->
...
$0.0361 lands in the same place, which is the check that the arithmetic is
...
**Caching does a lot of quiet work.** 187,968 of 225,513 text input tokens —
83% — were served from cache. At the uncached rate those would have cost
$0.1128 instead of $0.0113. The instructions and the tool schemas are
...
**What is invented, and is not evidence of anything.** The brief supplied the
problem and says explicitly that I need not invent a different business, so it
is worth being clear about which parts of the page are furniture. The name
"Aperture Rentals", the day rates of $75 / $35 / $30, and the three reassurance
lines under the headline ("tested and insured", "same-day pickup", "cancel
before pickup") were written by me. They are not researched market rates and
not claims any real desk has made. They exist so the agent has a plausible
counter to stand behind, and they all live in `src/config/brand.ts` and
`src/config/catalog.ts` so a reviewer can see their full extent in two files.
Nothing in the booking logic reads them: the price is never spoken, never
stored on a reservation, and never affects availability. Delete them and every
