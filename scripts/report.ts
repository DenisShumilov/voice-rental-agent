...
/** A gap longer than this between marks means the work stopped and restarted. */
...
function timeBlock(): string {
  const stamps = execFileSync('git', ['log', '--reverse', '--format=%aI'], { encoding: 'utf8' })
    .trim()
    .split('\n')
  ...
  // The first stretch has no commits in it: it is the reading, the architecture
  // and the provider checks done before the repository existed. It is declared
  // rather than derived, and the paragraph below says so.
  const sittings: Array<{ from: Date; to: Date; derived: boolean }> = [
    { from: start, to: stamps[0], derived: false },
  ]

  // After that, the work happened in sittings with a night in between. Counting
  // straight through would make sleep look like work, which is exactly the sort
  // of unmeasured figure this file exists to avoid, so a long gap starts a new
  // sitting instead of being counted.
  for (const mark of stamps) {
    const current = sittings[sittings.length - 1]
    if (mark.getTime() - current.to.getTime() > BREAK_MINUTES * 60_000) {
      sittings.push({ from: mark, to: mark, derived: true })
    } else {
      current.to = mark
    }
  }
  ...
  const worked =
    sittings.reduce((total, s) => total + (s.to.getTime() - s.from.getTime()) / 60_000, 0) -
    PAUSE_MINUTES
  const span = (stamps[stamps.length - 1].getTime() - start.getTime()) / 60_000
  ...
  let out = `**About ${hhmm(worked)} of work**, against an eight-hour ceiling.\n\n`
  out += `That is ${sittings.length === 1 ? 'one sitting' : `${sittings.length} sittings`}, `
  out += `not a single stretch: `
  out += sittings.map((s) => `${day(s.from)} ${clock(s.from)}–${clock(s.to)}`).join(', ')
  out += `. Counting straight through from the first mark to the last would give`
  out += ` ${hhmm(span)}, but that includes the night in between, and sleep is not work.`
  out += ` ${PAUSE_MINUTES} minutes of an enforced pause on my
`
  out += `AI tooling are subtracted on top of that.

`
  out += `Derived, not remembered: this paragraph is regenerated by \`npm run report\` from\n`
  out += `\`git log\`, and reads as of that run — ${stamps.length} commits. A gap longer than\n`
  out += `${BREAK_MINUTES} minutes between commits starts a new sitting. The first stretch is
`
  out += `the exception: it holds no commits, because it is the reading and the
`
  out += `architecture done before the repository existed. It starts at the creation time
`
  out += `of the first working file — a file timestamp, not a commit, and the one figure
`
  out += `here that git cannot corroborate.`
...
/**
 * How many tests there actually are, from a real run rather than a number
 * typed into four documents. It went stale the moment the suite grew, which is
 * the same failure the latency and cost tables were generated to avoid.
 */
function testsBlock(): string {
  const output = '.vitest/report.json'
  // The local binary rather than a shell: passing arguments through a shell
  // concatenates instead of escaping them, and this path contains spaces.
  ...
  const run = JSON.parse(readFileSync(output, 'utf8')) as {
  ...
  return (
    `**${run.numPassedTests} of ${run.numTotalTests} tests pass**, across ` +
    `${run.testResults.length} files, run by \`npm test\`. ` +
    `Counted by running them, not by remembering.`
  )
}
